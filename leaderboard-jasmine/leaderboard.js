...
PlayersService = {
  ...
  dec: function (playerId) {
    Players.update(playerId, {$inc: {score: -5}});
  },
  removeName: function (playerId) {
...
if (Meteor.isClient) {
  ...
  Template.leaderboard.events({
    ...
    'click input.dec': function () {
      PlayersService.dec(Session.get("selected_player"));
    },
      'click input.remove': function () {
      PlayersService.removeName(Session.get("selected_player"));
    },
    'submit form': function(event){
        event.preventDefault();
        PlayersService.AddName(event.target.playerName.value);
